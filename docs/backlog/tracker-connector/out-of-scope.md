...
- **A second agent runtime** — the connector never launches Claude Code or Codex itself. Anything that needs a model runs as a worc task inside worc's sandbox (that is what optional triage is). _Superseded as a planned option by the [local-research](../local-research/README.md) record (2026-09-11), which adds a second report producer behind a switch; this line, and the invariant behind it, hold until that record's phase 01 lands._
