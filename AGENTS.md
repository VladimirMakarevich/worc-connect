...
- **Deterministic code decides.** A model may propose (a triage report); only the connector's deterministic task builder produces a task from it, from a verdict block whose vocabulary is closed and whose absence ends the attempt visibly. No path promotes model output unreviewed by code.
- **Issue text is untrusted input from strangers.** It has exactly one destination: the task file body. It is never an argument, an environment value, a log line, a label, or a comment. Comment bodies are connector-authored templates that go to the tracker through a body **file**; ids and URLs are the only item-derived values that appear anywhere else. The one exception — the `reason` of a triage verdict, published as the agent wrote it — is recorded with its cost in `docs/backlog/follow-ups.md`.
...
- **The Markdown corpus is linted, and the gate must stay green.** `python tools/mdlint.py` checks the root files, [.agents/rules/](.agents/rules/), `.claude/skills/`, and `docs/`: relative links and anchors resolve, no document is unreachable, nothing outgrows its size budget. A new document has to be linked from somewhere; a document that lives in another repository is named in plain text, never linked. The linter is the pinned `@wastech-mdlint/cli` npm devDependency, so `npm ci` (Node >= 24.17) is the whole setup; it runs as a pre-commit hook and as its own CI job. Without it installed the runner prints how to install it and passes locally, and fails hard under `CI`.
