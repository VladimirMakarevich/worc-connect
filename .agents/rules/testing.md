...
- **worc is a test dependency, installed separately** (`pip install -r requirements-worc.txt`), and only one suite uses it: the generated task file is judged by worc's real `task.validation_gate` rather than by a second, local copy of its rules, which could drift from the real ones without anything failing. It is deliberately not in `pyproject.toml` — worc is published to no index, and a direct reference in the project's metadata is refused by the build backend and by every index. The suite carries `requires_worc` and skips where it is absent; CI always installs it. Nothing under `src/` may import it.
