...
1. **The invariant, rewritten (D1).** [AGENTS.md](../../../../AGENTS.md) and [.agents/rules/architecture.md](../../../../.agents/rules/architecture.md) — "The model boundary": replace "No agent runtime" with the narrow form (research only, behind `research.mode: local`, in a disposable worktree of the connector's own clone, never authoring a task). [.agents/rules/security.md](../../../../.agents/rules/security.md) gains the posture of [D12](../design.md#d12--the-blast-radius-is-the-worktree-and-that-is-the-whole-of-it): the gate is this feature's security perimeter. In [tracker-connector/out-of-scope.md](../../tracker-connector/out-of-scope.md), mark "A second agent runtime" as superseded by this record, and do the same for NFR-6 and R-5 there. Nothing is left saying two different things.
...
4. `cli.py` — the provider is resolved by the composition root from `research.mode` and injected into the loop; the `worc` provider is the phase 07 path (`core/triage.py`, `core/reconcile.py`) moved behind the protocol with no behaviour change, and a `local` provider stub that reports "not built yet" is acceptable only inside this phase's own branch and must not survive phase 03.
...
7. `.importlinter` — a contract forbidding `worc_connect.core` from importing `worc_connect.research`, and one keeping `worc_connect.research` off `cli` and `config`.

## Files touched

`AGENTS.md`, `.agents/rules/{architecture,security}.md`, `docs/backlog/tracker-connector/out-of-scope.md`, `README.md`, `.importlinter`, `src/worc_connect/{cli,config}.py`, `src/worc_connect/core/{research,triage,loop}.py`, `docs/configuration.md`, `tests/`.
...
- [ ] No document in the repository still claims the connector never launches an agent.
