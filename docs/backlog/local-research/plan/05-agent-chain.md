...
4. `outcome.json` records `attempts: [{name, exit_code, reason}]` and `produced_by`; `agent.log` is per attempt; the built task's provenance line and the item comment name the agent that produced the report. `core/builder.py`'s `build_from_report` already takes six arguments, which is exactly `max-args`, so the producing agent arrives in a parameter object with the other report-derived inputs — never as a seventh argument and never behind a per-file ignore.
