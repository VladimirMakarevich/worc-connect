...
2. The origin URL is read from the connector's own configuration (the `repo` pin) resolved through the tracker adapter's clone URL, not from worc's clone configuration — the connector still runs no `git` against worc's clone.
3. Every `git` launch is an argument list with an explicit `-C` or `--git-dir` naming the connector's own paths; `git` is resolved with `shutil.which`; stored paths go through `Path.as_posix()`.
4. `doctor` (from phase 01) grows the clone checks: present or creatable, fetchable, push URL disabled, worktree root writable, and a list of any orphaned worktrees it would sweep.
5. `keep_worktree: true` skips removal and says where the tree was left.
...
- [ ] A worktree can be created, used and removed without any process touching worc's clone.
