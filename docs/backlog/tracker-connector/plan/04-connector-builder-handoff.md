...
6. Add worc as a **test** dependency so AC-4 can run the generated file through the real `task.validation_gate`. Not in `pyproject.toml`: worc is published to no index, so it can only be named by URL, and a direct reference in the project's metadata is refused by the build backend and by every index. It lives in `requirements-worc.txt`, CI installs it as its own step, and the suite skips where it is absent.
...
- The generated file is judged by worc's **real** validation gate in the suite: worc is a test-only dependency, installed from its repository (`requirements-worc.txt`, a step of its own in CI) because it is on no package index and a direct reference in this project's metadata would be refused by the build backend and by every index; the test skips where it is absent.
