...
Exactly one of these is on an issue at a time; re-applying the one it already has does nothing. The prefix is yours to change (`write_back.labels_prefix`). On its first tick the connector creates whichever labels the repository is missing — these, and the trigger label(s) your gate names, so a fresh repository has something to gate with — and never edits one that already exists; the two triage labels are created only with `research.mode: worc`. A state label the connector did not write, with no worc task behind it, is left alone and logged as `state-label-without-task`: remove it to let the connector take that issue on.
